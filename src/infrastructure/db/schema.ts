...
import type { SubscriptionCataloguePayload } from "@/modules/subscriptions/model";
import {
...
import { AUDIT_ACTIONS, AUDIT_ACTOR_TYPES } from "@/modules/audit/model";
import { OUTBOX_STATUSES } from "@/modules/events/model";
import {
  NOTIFICATION_DESTINATION_TYPES,
  NOTIFICATION_TYPES,
} from "@/modules/notifications/model";
import { PARTNER_ACTIVITY_TYPES } from "@/modules/partners/model";
import {
  PUBLIC_MEDIA_ASSET_STATUSES,
  PUBLIC_MEDIA_TARGET_TYPES,
  type PublicMediaAssetStatus,
...
export const activityTypeEnum = pgEnum("activity_type", PARTNER_ACTIVITY_TYPES);
...
export const financialJournalEntryTypeEnum = pgEnum(
  "financial_journal_entry_type",
  FINANCIAL_JOURNAL_ENTRY_TYPES,
);
export const financialDirectionEnum = pgEnum(
  "financial_direction",
  FINANCIAL_DIRECTIONS,
);
export const financialChannelEnum = pgEnum(
  "financial_channel",
  FINANCIAL_CHANNELS,
);
...
export const publicMediaAssetStatusEnum = pgEnum(
  "public_media_asset_status",
  PUBLIC_MEDIA_ASSET_STATUSES,
);
export const publicMediaTargetTypeEnum = pgEnum(
  "public_media_target_type",
  PUBLIC_MEDIA_TARGET_TYPES,
);
...
export const typeNotificationEnum = pgEnum(
  "type_notification",
  NOTIFICATION_TYPES,
);
export const notificationDestinationTypeEnum = pgEnum(
...
export const payoutDestinationTypeEnum = pgEnum(
  "payout_destination_type",
  ["bank_account", "mobile_money"],
);

export const paymentProviderEnvironmentEnum = pgEnum(
...
// ============================================================================
// MEDIA — REGISTRE DES ASSETS PUBLICS
// ============================================================================
...
export const publicMediaAssets = pgTable(
  "public_media_assets",
  {
    ...
    sizeBytes: integer("size_bytes").notNull(),
    sha256: varchar("sha256", { length: 64 }).notNull(),
    status: publicMediaAssetStatusEnum("status")
      .notNull()
      .default("temporary")
      .$type<PublicMediaAssetStatus>(),
    targetType: publicMediaTargetTypeEnum("target_type").$type<PublicMediaTargetType>(),
  ...
  (table) => ({
    storageKeyUnique: uniqueIndex("public_media_assets_storage_key_unique").on(
      table.storageKey,
    ),
    publicUrlUnique: uniqueIndex("public_media_assets_public_url_unique").on(
      table.publicUrl,
    ),
    cleanupIdx: index("public_media_assets_cleanup_idx")
      .on(table.expiresAt, table.id)
      .where(sql`${table.status} IN ('temporary', 'deleting')`),
    targetIdx: index("public_media_assets_target_idx")
      .on(table.targetType, table.targetId)
      .where(sql`${table.status} = 'attached'`),
    contentTypeValid: check(
      "public_media_assets_content_type_valid",
      sql`${table.contentType} IN ('image/jpeg', 'image/png', 'image/webp')`,
    ),
    sizeValid: check(
      "public_media_assets_size_valid",
      sql`${table.sizeBytes} > 0 AND ${table.sizeBytes} <= ${5 * 1024 * 1024}`,
    ),
    sha256Valid: check(
      "public_media_assets_sha256_valid",
      sql`${table.sha256} ~ '^[0-9a-f]{64}$'`,
    ),
    lifecycleCoherent: check(
      "public_media_assets_lifecycle_coherent",
      sql`(${table.status} = 'temporary' AND ${table.targetType} IS NULL AND ${table.targetId} IS NULL AND ${table.expiresAt} IS NOT NULL AND ${table.attachedAt} IS NULL AND ${table.deletedAt} IS NULL)
        OR (${table.status} = 'attached' AND ${table.targetType} IS NOT NULL AND ${table.targetId} IS NOT NULL AND ${table.expiresAt} IS NULL AND ${table.attachedAt} IS NOT NULL AND ${table.deletedAt} IS NULL)
        OR (${table.status} = 'deleting' AND ${table.targetType} IS NULL AND ${table.targetId} IS NULL AND ${table.expiresAt} IS NOT NULL AND ${table.attachedAt} IS NULL AND ${table.deletedAt} IS NULL)
        OR (${table.status} = 'deleted' AND ${table.targetType} IS NULL AND ${table.targetId} IS NULL AND ${table.attachedAt} IS NULL AND ${table.deletedAt} IS NOT NULL)`,
    ),
  }),
);
...
export const paymentProviderAccounts = pgTable(
  "payment_provider_accounts",
  {
    ...
    destinationType: payoutDestinationTypeEnum("destination_type").notNull().default("bank_account"),
    providerEnvironment: paymentProviderEnvironmentEnum("provider_environment").notNull().default("live"),
    providerVerified: boolean("provider_verified").notNull().default(false),
    settlementInstitutionCode: varchar("settlement_institution_code", { length: 50 }),
    settlementInstitutionName: varchar("settlement_institution_name", { length: 160 }),
    accountIdentifierLast4: varchar("account_identifier_last4", { length: 4 }),
    verifiedAt: timestamp("verified_at", { withTimezone: true }),
    linkedByAdminId: varchar("linked_by_admin_id", { length: 36 })
      .references(() => users.id, { onDelete: "restrict" }),
    linkedByUserId: varchar("linked_by_user_id", { length: 36 })
      .references(() => users.id, { onDelete: "restrict" }),
  ...
  (table) => ({
    ...
    lifecycleCoherent: check(
      ...
      sql`(${table.status} IN ('pending', 'active') AND ${table.disabledAt} IS NULL) OR (${table.status} = 'disabled' AND ${table.disabledAt} IS NOT NULL)`,
    ),
    actorCoherent: check(
      "payment_provider_accounts_actor_coherent",
      sql`(CASE WHEN ${table.linkedByAdminId} IS NULL THEN 0 ELSE 1 END + CASE WHEN ${table.linkedByUserId} IS NULL THEN 0 ELSE 1 END) = 1`,
    ),
    verificationCoherent: check(
      "payment_provider_accounts_verification_coherent",
      sql`${table.providerVerified} = (${table.verifiedAt} IS NOT NULL)`,
    ),
    activeEligibility: check(
      "payment_provider_accounts_active_eligibility",
      sql`${table.status} <> 'active' OR ${table.providerEnvironment} = 'test' OR ${table.providerVerified}`,
    ),
    last4Valid: check(
      "payment_provider_accounts_last4_valid",
      sql`${table.accountIdentifierLast4} IS NULL OR ${table.accountIdentifierLast4} ~ '^[A-Za-z0-9]{4}$'`,
    ),
  }),
);
...
export const residences = pgTable(
  ...
  (table) => ({
    ...
    idPartnerAccountUnique: uniqueIndex(
      "residences_id_partner_account_unique",
    ).on(table.id, table.partnerAccountId),
...
export const residenceReservations = pgTable(
  ...
  (table) => ({
    ...
    residencePartnerFk: foreignKey({
      name: "residence_reservations_residence_partner_fk",
      columns: [table.residenceId, table.partnerAccountId],
      foreignColumns: [residences.id, residences.partnerAccountId],
    }).onDelete("restrict"),
  }),
);
...
export const creneauxHoraires = pgTable(
  ...
  (table) => ({
    ...
    restaurantIdUnique: uniqueIndex(
      "creneaux_horaires_restaurant_id_unique",
    ).on(table.restaurantId, table.id),
  })
);
...
export const categories = pgTable(
  "categories",
  {
    ...
    restaurantId: varchar("restaurant_id", { length: 36 })
      .notNull()
      .references(() => restaurants.id, { onDelete: "cascade" }),
    creneauId: varchar("creneau_id", { length: 36 }),
    nom: varchar("nom", { length: 255 }).notNull(),
    description: text("description"),
  ...
  (table) => ({
    ...
    restaurantIdUnique: uniqueIndex("categories_restaurant_id_unique").on(
      table.restaurantId,
      table.id,
    ),
    restaurantNameUnique: uniqueIndex("categories_restaurant_name_unique").on(
      table.restaurantId,
      sql`lower(trim(${table.nom}))`,
    ),
    scheduleOwnerFk: foreignKey({
      name: "categories_restaurant_schedule_fk",
      columns: [table.restaurantId, table.creneauId],
      foreignColumns: [
        creneauxHoraires.restaurantId,
        creneauxHoraires.id,
      ],
    }).onDelete("restrict"),
  })
...
export const plats = pgTable(
  "plats",
  {
    ...
    restaurantId: varchar("restaurant_id", { length: 36 })
      .notNull()
      .references(() => restaurants.id, { onDelete: "cascade" }),
    categorieId: varchar("categorie_id", { length: 36 }).notNull(),
    creneauId: varchar("creneau_id", { length: 36 }),
    nom: varchar("nom", { length: 255 }).notNull(),
    description: text("description"),
  ...
  (table) => ({
    ...
    categoryOwnerFk: foreignKey({
      name: "plats_restaurant_category_fk",
      columns: [table.restaurantId, table.categorieId],
      foreignColumns: [categories.restaurantId, categories.id],
    }).onDelete("restrict"),
    scheduleOwnerFk: foreignKey({
      name: "plats_restaurant_schedule_fk",
      columns: [table.restaurantId, table.creneauId],
      foreignColumns: [
        creneauxHoraires.restaurantId,
        creneauxHoraires.id,
...
export const commandes = pgTable(
  "commandes",
  {
    ...
    reconciliationState: varchar("reconciliation_state", { length: 24 })
      .$type<"complete" | "legacy_incomplete">()
      .notNull()
      .default("complete"),
    reconciliationIssues: text("reconciliation_issues")
      .array()
      .notNull()
  ...
  (table) => ({
    ...
    reconciliationStateValid: check(
      "commandes_reconciliation_state_valid",
      sql`${table.reconciliationState} IN ('complete', 'legacy_incomplete')`,
    ),
    reconciliationIssuesCoherent: check(
      "commandes_reconciliation_issues_coherent",
      sql`(${table.reconciliationState} = 'complete' AND cardinality(${table.reconciliationIssues}) = 0)
        OR (${table.reconciliationState} = 'legacy_incomplete' AND cardinality(${table.reconciliationIssues}) > 0)`,
    ),
  })
);
...
export const financialTransactions = pgTable(
  "transactions",
  {
    ...
    refundIdempotencyKey: varchar("refund_idempotency_key", { length: 128 }),
    paidAt: timestamp("paid_at", { withTimezone: true }),
    cancelledAt: timestamp("cancelled_at", { withTimezone: true }),
    createdAt: timestamp("created_at", { withTimezone: true })
      .notNull()
  ...
  (table) => ({
    ...
    refundIdempotencyUnique: uniqueIndex(
      "transactions_refund_idempotency_unique",
    )
      .on(table.originalPaymentId, table.refundIdempotencyKey)
      .where(sql`${table.originalPaymentId} IS NOT NULL`),
    originalPaymentIdx: index("transactions_original_payment_idx").on(
    ...
    sourceCoherent: check(
      ...
      sql`(${table.type} = 'commande_restaurant' AND ${table.restaurantOrderId} IS NOT NULL AND ${table.subscriptionRequestId} IS NULL AND ${table.commissionSettlementId} IS NULL AND ${table.residenceReservationId} IS NULL AND ${table.originalPaymentId} IS NULL AND ${table.refundIdempotencyKey} IS NULL)
        OR (${table.type} = 'abonnement_partenaire' AND ${table.restaurantOrderId} IS NULL AND ${table.subscriptionRequestId} IS NOT NULL AND ${table.commissionSettlementId} IS NULL AND ${table.residenceReservationId} IS NULL AND ${table.originalPaymentId} IS NULL AND ${table.refundIdempotencyKey} IS NULL)
        OR (${table.type} = 'commission_settlement' AND ${table.restaurantOrderId} IS NULL AND ${table.subscriptionRequestId} IS NULL AND ${table.commissionSettlementId} IS NOT NULL AND ${table.residenceReservationId} IS NULL AND ${table.originalPaymentId} IS NULL AND ${table.refundIdempotencyKey} IS NULL)
        OR (${table.type} = 'reservation_residence' AND ${table.restaurantOrderId} IS NULL AND ${table.subscriptionRequestId} IS NULL AND ${table.commissionSettlementId} IS NULL AND ${table.residenceReservationId} IS NOT NULL AND ${table.originalPaymentId} IS NULL AND ${table.refundIdempotencyKey} IS NULL)
        OR (${table.type} = 'remboursement' AND ${table.restaurantOrderId} IS NULL AND ${table.subscriptionRequestId} IS NULL AND ${table.commissionSettlementId} IS NULL AND ${table.residenceReservationId} IS NULL AND ${table.originalPaymentId} IS NOT NULL AND ${table.refundIdempotencyKey} IS NOT NULL)`,
    ),
    lifecycleCoherent: check(
...
export const payments = pgTable(
  "payments",
  {
    ...
    recordedReference: varchar("recorded_reference", { length: 255 }),
...
export const businessEvents = pgTable(
  ...
  (table) => ({
    correlationIdx: index("business_events_correlation_idx").on(
      table.correlationId,
    ),
    partnerIdx: index("business_events_partner_account_idx").on(
      table.partnerAccountId,
    ),
    targetIdx: index("business_events_target_idx").on(
      table.targetType,
      table.targetId,
    ),
    occurredAtIdx: index("business_events_occurred_at_idx").on(
      table.occurredAt,
    ),
    retainedUntilIdx: index("business_events_retained_until_idx").on(
      table.retainedUntil,
    ),
    payloadSize: check(
      "business_events_payload_size",
      sql`octet_length(${table.payload}::text) <= 16384`,
    ),
  }),
);
...
export const financialJournalEntries = pgTable(
  "financial_journal_entries",
  {
    ...
    partnerAccountId: uuid("partner_account_id")
      .notNull()
      .references(() => partnerAccounts.id, { onDelete: "restrict" }),
    subscriptionPeriodId: varchar("subscription_period_id", {
      length: 36,
    }).references(() => subscriptionPeriods.id, { onDelete: "restrict" }),
    eventId: uuid("event_id")
      .notNull()
      .references(() => businessEvents.id, { onDelete: "restrict" }),
    entryType: financialJournalEntryTypeEnum("entry_type").notNull(),
    direction: financialDirectionEnum("direction").notNull(),
    channel: financialChannelEnum("channel").notNull(),
    amountFcfa: integer("amount_fcfa").notNull(),
    currency: varchar("currency", { length: 3 }).notNull().default("XOF"),
    provider: varchar("provider", { length: 50 }),
  ...
  (table) => ({
    eventUnique: uniqueIndex("financial_journal_entries_event_unique").on(
      table.eventId,
    ),
    confirmedPaymentUnique: uniqueIndex(
      "financial_journal_entries_confirmed_payment_unique",
    )
      .on(table.paymentId)
      .where(sql`${table.entryType} = 'payment_confirmed'`),
    subscriptionPeriodUnique: uniqueIndex(
      "financial_journal_entries_subscription_period_unique",
    )
      .on(table.subscriptionPeriodId)
      .where(sql`${table.subscriptionPeriodId} IS NOT NULL`),
    partnerOccurredIdx: index(
      "financial_journal_entries_partner_occurred_idx",
    ).on(table.partnerAccountId, table.occurredAt),
    transactionIdx: index("financial_journal_entries_transaction_idx").on(
      table.transactionId,
    ),
    amountPositive: check(
      "financial_journal_entries_amount_positive",
      sql`${table.amountFcfa} > 0`,
    ),
    currencyXof: check(
      "financial_journal_entries_currency_xof",
      sql`${table.currency} = 'XOF'`,
    ),
    shapeCoherent: check(
      "financial_journal_entries_shape_coherent",
      sql`(${table.entryType} = 'payment_confirmed' AND ${table.paymentId} IS NOT NULL AND ${table.direction} = 'inflow' AND ${table.method} IS NOT NULL)
        OR (${table.entryType} = 'refund_obligation_created' AND ${table.paymentId} IS NOT NULL AND ${table.direction} = 'outflow' AND ${table.channel} = 'internal' AND ${table.subscriptionPeriodId} IS NULL)`,
    ),
  }),
);
...
export const outboxMessages = pgTable(
  ...
  (table) => ({
    eventEffectUnique: uniqueIndex("outbox_messages_event_effect_unique").on(
      table.eventId,
      table.effectType,
    ),
    claimIdx: index("outbox_messages_claim_idx").on(
      table.status,
      table.availableAt,
      table.createdAt,
    ),
    unresolvedIdx: index("outbox_messages_unresolved_idx")
      .on(table.createdAt)
      .where(sql`${table.status} = 'dead_letter'`),
    attemptsValid: check(
      "outbox_messages_attempts_valid",
      sql`${table.attempts} >= 0 AND ${table.maxAttempts} > 0`,
    ),
    payloadSize: check(
      "outbox_messages_payload_size",
      sql`octet_length(${table.payload}::text) <= 16384`,
    ),
    lifecycleCoherent: check(
      "outbox_messages_lifecycle_coherent",
      sql`(${table.status} = 'processing' AND ${table.lockedAt} IS NOT NULL AND ${table.completedAt} IS NULL)
        OR (${table.status} IN ('pending', 'retry') AND ${table.lockedAt} IS NULL AND ${table.completedAt} IS NULL)
        OR (${table.status} = 'completed' AND ${table.lockedAt} IS NULL AND ${table.completedAt} IS NOT NULL)
        OR (${table.status} = 'dead_letter' AND ${table.lockedAt} IS NULL AND ${table.deadLetteredAt} IS NOT NULL AND ${table.completedAt} IS NULL)`,
    ),
  }),
);

export const eventEffectReceipts = pgTable(
  "event_effect_receipts",
  {
    id: uuid("id")
      .primaryKey()
      .$defaultFn(() => crypto.randomUUID()),
    eventId: uuid("event_id")
      ...
      .references(() => businessEvents.id, { onDelete: "cascade" }),
    effectType: varchar("effect_type", { length: 80 }).notNull(),
    effectPayload: jsonb("effect_payload").$type<Record<string, unknown>>(),
    completedAt: timestamp("completed_at", { withTimezone: true })
      .notNull()
      .$defaultFn(() => new Date()),
  },
  (table) => ({
    eventEffectUnique: uniqueIndex("event_effect_receipts_event_effect_unique").on(
      table.eventId,
      table.effectType,
    ),
    completedAtIdx: index("event_effect_receipts_completed_at_idx").on(
      table.completedAt,
    ),
  }),
);
...
export const notifications = pgTable(
  "notifications",
  {
    ...
    // Destination métier typée. Une destination supprimée supprime sa projection.
    lienType: notificationDestinationTypeEnum("lien_type"),
    lienId: varchar("lien_id", { length: 128 }),
    eventId: uuid("event_id").references(() => businessEvents.id, {
      onDelete: "restrict",
    }),
  ...
  (table) => ({
    ...
    correlationIdx: index("notifications_correlation_idx").on(
      table.correlationId,
    ),
    eventUserUnique: uniqueIndex("notifications_event_user_unique")
      .on(table.eventId, table.userId)
      .where(sql`${table.eventId} IS NOT NULL AND ${table.userId} IS NOT NULL`),
    eventClientUnique: uniqueIndex("notifications_event_client_unique")
      .on(table.eventId, table.clientId)
      .where(sql`${table.eventId} IS NOT NULL AND ${table.clientId} IS NOT NULL`),
    eventDriverUnique: uniqueIndex("notifications_event_driver_unique")
      .on(table.eventId, table.driverId)
      .where(sql`${table.eventId} IS NOT NULL AND ${table.driverId} IS NOT NULL`),
    singleOwner: check(
      "notifications_single_owner",
      sql`num_nonnulls(${table.userId}, ${table.clientId}, ${table.driverId}) = 1`,
    ),
    destinationCoherent: check(
      "notifications_destination_coherent",
      sql`(${table.lienType} IS NULL) = (${table.lienId} IS NULL)`,
    ),
    eventCorrelationCoherent: check(
      "notifications_event_correlation_coherent",
      sql`(${table.eventId} IS NULL) = (${table.correlationId} IS NULL)`,
    ),
  })
);
...
export const auditLog = pgTable(
  "audit_log",
  {
    ...
    ressourceId: varchar("ressource_id", { length: 128 }).notNull(),
  ...
  (table) => ({
    ...
    actorCoherent: check(
      "audit_log_actor_coherent",
      sql`(${table.actorType} = 'admin' AND ${table.adminId} IS NOT NULL AND ${table.actorId} = ${table.adminId}) OR (${table.actorType} <> 'admin' AND ${table.adminId} IS NULL)`,
    ),
    eventCorrelationCoherent: check(
      "audit_log_event_correlation_coherent",
      sql`(${table.eventId} IS NULL) = (${table.correlationId} IS NULL)`,
    ),
  })
);
...
export const paymentProviderAccountsRelations = relations(paymentProviderAccounts, ({ one }) => ({
  ...
  linkedByUser: one(users, {
    fields: [paymentProviderAccounts.linkedByUserId],
    references: [users.id],
  }),
}));
...
export const categoriesRelations = relations(categories, ({ one, many }) => ({
  ...
  creneau: one(creneauxHoraires, {
    fields: [categories.restaurantId, categories.creneauId],
    references: [creneauxHoraires.restaurantId, creneauxHoraires.id],
  }),
  plats:      many(plats),
  promotions: many(promotions),
}));
...
export const platsRelations = relations(plats, ({ one, many }) => ({
  ...
  categorie: one(categories, {
    fields: [plats.restaurantId, plats.categorieId],
    references: [categories.restaurantId, categories.id],
  }),
  creneau: one(creneauxHoraires, {
    fields: [plats.restaurantId, plats.creneauId],
    references: [creneauxHoraires.restaurantId, creneauxHoraires.id],
  }),
  promotions: many(promotions),
}));
...
export const financialTransactionsRelations = relations(
  ...
  ({ one, many }) => ({
    ...
    originalPayment: one(payments, {
      fields: [financialTransactions.originalPaymentId],
      references: [payments.id],
      relationName: "refund_original_payment",
    }),
    payments: many(payments, { relationName: "transaction_payment_attempts" }),
    journalEntries: many(financialJournalEntries),
  }),
);
...
export const paymentsRelations = relations(payments, ({ one, many }) => ({
  transaction: one(financialTransactions, {
    ...
    relationName: "transaction_payment_attempts",
  }),
  ...
  refundObligations: many(financialTransactions, {
    relationName: "refund_original_payment",
  }),
}));

export const financialJournalEntriesRelations = relations(
  financialJournalEntries,
  ({ one }) => ({
    transaction: one(financialTransactions, {
      fields: [financialJournalEntries.transactionId],
      references: [financialTransactions.id],
    }),
    payment: one(payments, {
      fields: [financialJournalEntries.paymentId],
      references: [payments.id],
    }),
    partnerAccount: one(partnerAccounts, {
      fields: [financialJournalEntries.partnerAccountId],
      references: [partnerAccounts.id],
    }),
    subscriptionPeriod: one(subscriptionPeriods, {
      fields: [financialJournalEntries.subscriptionPeriodId],
      references: [subscriptionPeriods.id],
    }),
    event: one(businessEvents, {
      fields: [financialJournalEntries.eventId],
