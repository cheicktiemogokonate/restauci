...
async function getActor() {
  ...
  const partnerAccount = await getPartnerAccountByUserId(user.userId);
  if (!partnerAccount || partnerAccount.activityType !== "restaurant") {
    throw new Error("Compte restaurant introuvable");
  }
  ...
  return { userId: user.userId, restaurantId: restaurant.id };
}
