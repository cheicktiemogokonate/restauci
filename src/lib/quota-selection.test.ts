...
import { selectQuotaEligibleResources } from "./quota-selection";
...
const dish = (id: string, categorieId: string, firstPublishedAt: Date | null = publishedAt) => ({
  id,
  categorieId,
  createdAt,
  firstPublishedAt,
...
describe("quota publication selection", () => {
  it("uses category slots before ranking dishes", () => {
    const result = selectQuotaEligibleResources(
  ...
  it("does not release a slot when publication intent is turned off", () => {
    ...
    const result = selectQuotaEligibleResources([category("cat-a")], resources, {
      category: 1,
      dish: 1,
    });
  ...
  it("releases a slot only after deletion", () => {
    const before = selectQuotaEligibleResources(
      [category("cat-a")],
      [dish("dish-a", "cat-a"), dish("dish-b", "cat-a")],
      { category: 1, dish: 1 },
    );
    const after = selectQuotaEligibleResources(
      [category("cat-a")],
      [dish("dish-b", "cat-a")],
      { category: 1, dish: 1 },
    );
  ...
  it("reacts deterministically to quota increases and decreases", () => {
    ...
    const large = selectQuotaEligibleResources([category("cat-a")], dishes, { category: 1, dish: 3 });
    const small = selectQuotaEligibleResources([category("cat-a")], dishes, { category: 1, dish: 1 });
