...
import { ResidenceCalendarManager } from "./residence-calendar-manager";
...
import type { ResidenceReservationActions } from "./action-types";
...
function ReservationActions({
  ...
  reservation: ResidenceReservationDTO;
  unavailable: ResidenceAvailabilityDTO["unavailable"];
  actions: ResidenceReservationActions;
}) {
  ...
  const cancel = () => {
    ...
    startTransition(async () => {
      ...
      if (result.refundObligationId) toast.warning(result.message);
          ...
          <div className="space-y-4">
            {isPaid ? (
              <Alert variant="destructive">
                ...
                <AlertDescription>
                  L’annulation enregistrera immédiatement une obligation de
                  remboursement intégral dans le suivi financier Toutci.
                </AlertDescription>
              </Alert>
            ) : null}
            <div className="space-y-2">
...
function ReservationsPanel({
  ...
  const columns: TableColumn<ResidenceReservationDTO>[] = [
    {
      key: "residenceTitle",
      header: "Résidence",
      sortable: true,
      width: "190px",
      cell: (reservation) => (
        <span className="font-medium">{reservation.residenceTitle}</span>
      ),
    },
    {
      key: "clientName",
    ...
    <>
      <div className="hidden md:block">
        <Table
          ...
          rowHeight={72}
          height={Math.min(624, Math.max(264, reservations.length * 72 + 48))}
          className="rounded-xl bg-card"
          emptyState="Aucune réservation reçue."
        />
      </div>
...
export function PartnerReservationsWorkspace({
  ...
  const orderedReservations = useMemo(() => {
    if (!focusReservationId) return reservations;
    return reservations.slice().sort((first, second) => {
      if (first.id === focusReservationId) return -1;
      if (second.id === focusReservationId) return 1;
      return 0;
    });
  }, [focusReservationId, reservations]);
  const focusedReservation = orderedReservations.find(
      ...
      <TabsContent value="reservations" className="mt-6 outline-none">
        {focusedReservation ? (
          <Alert className="mb-5 border-emerald-200 bg-emerald-50 text-emerald-950">
            <CalendarDays />
            <AlertTitle>Réservation ouverte depuis vos notifications</AlertTitle>
            <AlertDescription>
              {focusedReservation.residenceTitle} · {formatStayDate(focusedReservation.checkIn)} → {formatStayDate(focusedReservation.checkOut)}
            </AlertDescription>
          </Alert>
        ) : null}
        <div className="mb-4">
