...
import { calculateArrearsRecovery, resolveCashAccess } from "../model";
import { sendNotification } from "@/modules/notifications/server";
import { getCommissionRateBps } from "@/modules/subscriptions/server";
...
async function createSettlementReservationInTransaction(
  tx: CommissionTx,
  input: {
    partnerAccountId: string;
    amountFcfa: number;
    source: "paystack_direct" | "manual_admin";
    reference: string;
...
export async function createPaystackSettlementInTransaction(
  ...
  return createSettlementReservationInTransaction(tx, {
    ...input,
...
export async function confirmReservedSettlementInTransaction(
  ...
  if (settlement.statut === "confirmed") {
    const allocations = await tx
      .select({
        commissionId: commissionSettlementAllocations.commissionId,
        amountFcfa: commissionSettlementAllocations.amountFcfa,
      })
      .from(commissionSettlementAllocations)
      .where(eq(commissionSettlementAllocations.settlementId, settlement.id));
    return { settlement, allocations, alreadyConfirmed: true };
  }
...
export async function createManualCommissionSettlement(input: {
  ...
  try {
    return await transactionalDb.transaction(async (tx) => {
      ...
      const settlement = await createSettlementReservationInTransaction(tx, {
        partnerAccountId: input.partnerAccountId,
      ...
      const confirmation = await confirmReservedSettlementInTransaction(
        tx,
        settlement.id,
        input.paidAt,
      );
      const remainingDebt = await getOutstandingDebtWith(tx, input.partnerAccountId);
