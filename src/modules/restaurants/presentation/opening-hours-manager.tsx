...
import type { OpeningHoursInput, RestaurantScheduleDTO } from "../contracts";
import type { RestaurantScheduleActions } from "./action-types";
...
export default function OpeningHoursManager({
  ...
  initialCreneaux: RestaurantScheduleDTO[];
  actions: RestaurantScheduleActions;
}) {
  ...
  const openEdit = (creneau: RestaurantScheduleDTO) => {
    setForm({
      id: creneau.id,
      nom: creneau.nom,
  ...
  const toggle = (id: string, actif: boolean) => {
    ...
    startTransition(async () => {
      const result = await actions.toggle(id, actif);
      if (result.error) {
  ...
  const remove = () => {
    ...
    startTransition(async () => {
      const result = await actions.remove(currentId);
      if (result.error) {
        toast.error(result.error);
        return;
      }
            ...
            {creneaux.map((creneau) => (
              <div key={creneau.id} className="flex flex-col gap-3 p-3.5 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex items-center justify-between gap-1 sm:justify-end">
                  <Switch checked={creneau.actif} onCheckedChange={(checked) => toggle(creneau.id, checked)} disabled={isPending} ariaLabel={`Activer ${creneau.nom}`} className="data-checked:bg-brand-green" />
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[90vh] w-[calc(100%-2rem)] max-w-lg overflow-y-auto rounded-2xl p-5 sm:p-6">
          ...
          <div className="space-y-5">
            <div className="grid gap-2"><Label htmlFor="creneau-nom">Nom du créneau</Label><Input id="creneau-nom" value={form.nom} onChange={(nom) => setForm((current) => ({ ...current, nom }))} placeholder="Ex. Déjeuner" disabled={isPending} /></div>
            <div className="grid grid-cols-2 gap-3"><div className="grid gap-2"><Label htmlFor="creneau-ouverture">Ouverture</Label><Input id="creneau-ouverture" type="time" value={form.heureOuverture} onChange={(heureOuverture) => setForm((current) => ({ ...current, heureOuverture }))} disabled={isPending} /></div><div className="grid gap-2"><Label htmlFor="creneau-fermeture">Fermeture</Label><Input id="creneau-fermeture" type="time" value={form.heureFermeture} onChange={(heureFermeture) => setForm((current) => ({ ...current, heureFermeture }))} disabled={isPending} /></div></div>
