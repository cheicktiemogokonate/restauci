...
import { RESTAURANT_ORDER_STATUS_LABELS } from "@/modules/orders/model";
...
import { getAdminRestaurantStatus } from "../model";
import type {
  AdminRestaurantActions,
  AdminRestaurantDetailViewModel,
  AdminRestaurantOrderViewModel,
} from "./admin-types";

interface RestaurantDetailAdminProps {
  restaurant: AdminRestaurantDetailViewModel;
  commandes: AdminRestaurantOrderViewModel[];
  totalCommandes: number;
  page: number;
  totalPages: number;
  evolution: { jour: string; count: number; total: number }[];
  actions: AdminRestaurantActions;
}
...
function CommandeStatutBadge({ statut }: { statut: string }) {
  const label =
    RESTAURANT_ORDER_STATUS_LABELS[
      statut as keyof typeof RESTAURANT_ORDER_STATUS_LABELS
    ] ?? statut;
  return (
    <StatusBadge
      variant={
...
export function RestaurantDetailAdmin({
  ...
  const handleValider = () =>
    startTransition(async () => {
      try {
        const result = await actions.validate(restaurant.id);
  ...
  const handleRejeter = () => {
    ...
    startTransition(async () => {
      try {
        const result = await actions.reject(restaurant.id, motif);
        if ("error" in result) throw new Error(result.error);
  ...
  const handleSuspendre = () => {
    ...
    startTransition(async () => {
      try {
        const result = await actions.suspend(restaurant.id, motif);
        if ("error" in result) throw new Error(result.error);
  ...
  const handleReactiver = () =>
    startTransition(async () => {
      try {
        const result = await actions.reactivate(restaurant.id);
        if ("error" in result) throw new Error(result.error);
  ...
  const commandeColumns: TableColumn<AdminRestaurantOrderViewModel>[] = [
    {
      key: "numero",
      header: "N°",
      sortable: true,
