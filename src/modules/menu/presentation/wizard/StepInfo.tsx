...
interface StepInfoProps {
  ...
  onImageChange: (image: { assetId: string; url: string } | null) => void;
  inputCls: string;
}
...
export default function StepInfo({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      onImageChange({ assetId: data.assetId, url: data.url });
    } catch {
    ...
    <div className="max-w-150 space-y-5">
      <div>
        ...
        <Input
          name="nom"
          value={formData.nom}
          onChange={(value) => onFieldChange("nom", value)}
      ...
      <div>
        ...
        <div className="relative">
          <Input
            name="prix"
            value={formData.prix}
            onChange={(value) => onFieldChange("prix", value)}
            placeholder="Ex: 3500"
            leftIcon={<Banknote />}
            classNames={{ field: inputCls, input: "pl-11 pr-16" }}
          />
