...
import type { MenuCategoryActions } from "./action-types";
...
export default function CategoryOrganizerDialog({
  ...
  const saveRename = (categoryId: string) => {
    ...
    startTransition(async () => {
      const result = await actions.rename(categoryId, name);
      if (result.error) {
          ...
          <div className="rounded-xl border border-dashed border-brand-green/25 bg-brand-green/[0.035] p-3">
            ...
            <div className="flex gap-2">
              <Input
                ...
                disabled={isPending}
                classNames={{ field: "h-10 bg-background" }}
              />
              <Button
                type="button"
                    ...
                    <div className="min-w-0 flex-1">
                      {isEditing ? (
                        <Input
                          ...
                          autoFocus
                          disabled={isPending}
                          classNames={{ field: "h-8 rounded-lg" }}
                        />
                      ) : (
                    ...
                    {isEditing ? (
                      ...
                      <>
                        <Switch
                          ...
                          disabled={isPending}
                          ariaLabel={`Publication de ${category.nom}`}
                          onCheckedChange={(checked) => startTransition(async () => {
                            const result = await actions.setPublication(category.id, checked);
                            if (result.error) {
                              toast.error(result.error);
                              return;
                            }
