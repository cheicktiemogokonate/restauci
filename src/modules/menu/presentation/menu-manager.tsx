...
import type {
  MenuCategoryManagementDTO,
  MenuDishDTO,
} from "@/modules/menu/contracts";
...
import type { MenuCategoryActions, MenuDishActions } from "./action-types";

interface MenuManagerProps {
  categories: MenuCategoryManagementDTO[];
  initialPlats: MenuDishDTO[];
  ...
  categoryActions: MenuCategoryActions;
  dishActions: MenuDishActions;
}
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {initialPlats.map((plat) => (
                <MenuCard key={plat.id} plat={plat} actions={dishActions} />
              ))}
            </div>
