...
const ScrollStack = ({
  ...
  const getElementOffset = useCallback(
    (element: HTMLElement) => {
      // If we are measuring a transformed scroll-stack-card, measure its wrapper parent instead.
      // This is crucial to avoid layout feedback loops since CSS transforms mutate the element's bounding rect.
      const targetElement = (element.classList.contains("scroll-stack-card") && element.parentElement)
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    // Scoped query of the end marker relative to the current scroller instance to avoid global collision
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      ...
      const lenis = new Lenis({
        ...
        syncTouch: true,
        syncTouchLerp: 0.075,
      } as any); // cast config to bypass slight type differences
  ...
  useLayoutEffect(() => {
    ...
    // Scoped container query prevents targeting cards across multiple stack components
    ...
    cards.forEach((card, i) => {
      ...
      if (i < cards.length - 1 && parent) {
        parent.style.marginBottom = `${itemDistance}px`;
      }
