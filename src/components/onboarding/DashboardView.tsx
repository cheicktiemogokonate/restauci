...
export default function DashboardView({ config, onReset, onEdit }: DashboardViewProps) {
  return (
    <div className="flex-1 bg-gray-50/50 min-h-screen px-4 py-8 lg:py-12 flex flex-col items-center justify-center font-sans antialiased">
      ...
      <div className="w-full max-w-4xl bg-white border border-gray-100/80 rounded-3xl shadow-xs overflow-hidden">
        ...
        {/* Banner with Logo */}
        <div className="relative h-48 w-full bg-gradient-to-r from-brand-500 to-brand-600 overflow-hidden">
        ...
        <div className="p-6 md:p-8 space-y-8">
          ...
          <div className="p-4 bg-emerald-50/40 text-emerald-800 border border-emerald-100/50 rounded-2xl flex items-start space-x-3.5 text-xs md:text-sm">
            ...
            <div>
              <span className="font-bold font-display block text-emerald-950">Félicitations, l'onboarding est complété !</span>
              <p className="text-[12.5px] text-emerald-700 font-sans mt-0.5 leading-relaxed">
                Les coordonnées géographiques, les horaires d'ouverture et le profil numérique de <strong>{config.general.name || "votre établissement"}</strong> ont été figés et enregistrés sur votre terminal.
              </p>
            </div>
          </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="bg-gray-50/50 border border-gray-100/70 p-4 rounded-2xl space-y-3.5 text-sm">
                  <div>
                    <span className="text-[10px] font-mono font-bold text-gray-400 uppercase block leading-none">Description</span>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-2.5 border-t border-gray-100/70 text-xs">
                    <div>
                      <span className="text-[10px] font-mono font-bold text-gray-400 uppercase block">Catégorie</span>
                      <span className="text-gray-900 font-semibold uppercase font-display mt-0.5 block">{config.settings.category || "Bistrot"}</span>
                    </div>
                    <div>
                      <span className="text-[10px] font-mono font-bold text-gray-400 uppercase block">Monnaie & TVA</span>
                      <span className="text-gray-900 font-mono font-semibold mt-0.5 block">{config.settings.currency} (TVA {config.settings.taxRate}%)</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="bg-gray-50/50 border border-gray-100/70 p-4 rounded-2xl space-y-3 text-xs leading-relaxed">
                  <div className="flex items-start">
                    <span className="w-20 text-gray-400 shrink-0 font-medium">Zone</span>
                    <span className="text-gray-800 font-semibold">{config.address.commune}, {config.address.city} • {config.address.country}</span>
                  </div>
                  <div className="flex items-start">
                    <span className="w-20 text-gray-400 shrink-0 font-medium">Adresse</span>
                    <span className="text-gray-800 font-medium leading-normal">{config.address.fullAddress}</span>
                  </div>
                  <div className="flex items-start pt-2 border-t border-gray-100/70">
                    <span className="w-20 text-gray-400 shrink-0 font-medium">Coordonnées</span>
                    <span className="text-gray-900 font-mono font-bold flex items-center gap-1.5">
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                      LAT {config.address.latitude.toFixed(5)} • LNG {config.address.longitude.toFixed(5)}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="bg-gray-50/50 border border-gray-100/70 p-4 rounded-2xl space-y-2">
                  {config.schedule.map((row) => (
                    <div key={row.day} className="flex justify-between items-center text-xs font-sans py-0.5">
                ...
                <div className="bg-gray-50/50 border border-gray-100/70 p-4 rounded-2xl space-y-2.5 text-xs font-sans">
                  {config.address.phone && (
                    <div className="flex items-center space-x-2 text-gray-700">
                      ...
                      <span className="font-semibold">{config.address.phone}</span>
                    </div>
                  )}
                  {config.address.email && (
                    <div className="flex items-center space-x-2 text-gray-700">
                      <Mail className="w-3.5 h-3.5 text-gray-450 shrink-0" />
                      <span className="font-semibold">{config.address.email}</span>
                    </div>
                  )}
                  ...
                  <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-100/70">
                    {config.socials.instagram && (
                      <span className="inline-flex items-center space-x-1 px-2.5 py-1 bg-gradient-to-r from-purple-500/10 to-pink-500/10 text-pink-700 rounded-lg text-[10.5px] font-semibold border border-pink-100/50">
                    ...
                    {!config.socials.instagram && !config.socials.facebook && !config.socials.website && (
                      <span className="text-gray-400 italic text-[11px]">Aucun réseau social lié.</span>
                    )}
                  </div>
                </div>
              </div>
          ...
          <div className="pt-6 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between text-xs text-gray-400 gap-4">
            <span>RestauCI Onboarding Engine © 2026. Tous droits réservés.</span>
            <div className="flex items-center space-x-4">
              <button
                type="button"
                onClick={onEdit}
