...
export interface BottomSheetProps {
  ...
  /** Render relative to a device frame instead of the viewport. */
  embedded?: boolean;
  /** Optional externally-driven snap index for scroll narratives. */
  controlledSnap?: number;
  /** Leave height ownership to a single external GSAP timeline. */
  timelineControlled?: boolean;
  /** Keep a scripted sheet visible and non-dismissible. */
  dismissible?: boolean;
}
...
export function BottomSheet({
  ...
  const activeSnap = controlledSnap ?? snap;
  const snapValue = snapPoints[Math.min(activeSnap, snapPoints.length - 1)];
  const heightStyle =
    snapValue === "auto"
      ? { maxHeight: embedded ? "92%" : "92vh" }
      : { height: `${snapValue * 100}${embedded ? "%" : "vh"}` };
  ...
  const content = (
    <AnimatePresence initial={embedded ? false : undefined}>
      {open && dismissible ? (
        <PresenceGate key="backdrop">
          {({ gate }) => (
            <motion.button
              ...
              className={cn(
                "pointer-events-auto inset-0 z-50 bg-background/40 backdrop-blur-sm",
      ...
      {open && !dismissible ? (
        <motion.div
          ...
          transition={DRAWER}
          className={cn(
            "pointer-events-none inset-0 z-50 bg-background/40 backdrop-blur-sm",
            embedded ? "absolute" : "fixed",
          )}
        />
      ) : null}
        ...
        <PresenceGate key="sheet">
          {({ gate }) => (
            <motion.div
              ...
              {...gate}
              style={{ ...(timelineControlled ? {} : heightStyle), ...gate.style }}
              className={cn(
                "pointer-events-auto bottom-0 left-0 right-0 z-50 mx-auto flex max-w-2xl flex-col overflow-hidden rounded-t-3xl will-change-transform",
                !embedded && "transition-[height] duration-500",
                embedded ? "absolute" : "fixed",
