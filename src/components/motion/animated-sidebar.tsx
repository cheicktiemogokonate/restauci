"use client";
/* eslint-disable react-hooks/set-state-in-effect, react-hooks/immutability, @typescript-eslint/no-empty-object-type */
  ...
  return (
    ...
    >
      <div
        ...
        className={cn(
          "group/sidebar-wrapper flex min-h-screen w-full min-w-0",
          className,
        )}
      >
        {children}
  ...
  return createPortal(
    ...
    >
      ...
      <motion.div
        ...
        className={cn(
          "pointer-events-auto fixed inset-y-0 flex h-screen w-(--sidebar-width-mobile) max-w-[88vw] flex-col overflow-hidden",
    ...
    return (
      ...
      >
        <motion.div
          ...
          className={cn(
            "sticky top-0 flex h-screen w-full flex-col overflow-hidden bg-background",
            collapsible === "offcanvas" && "w-[var(--sidebar-width)]",
            variant === "sidebar" &&
              (side === "left" ? "border-border border-r" : "border-border border-l"),
            variant === "floating" &&
              "m-2 h-[calc(100vh-1rem)] rounded-2xl border border-border shadow-sm",
            variant === "inset" && "m-2 h-[calc(100vh-1rem)] rounded-2xl",
            panelClassName,
          )}
        >
          <AnimatedSidebarPanelContext.Provider
